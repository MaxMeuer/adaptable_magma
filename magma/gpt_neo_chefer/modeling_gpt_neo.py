# https://github.com/finetuneanon/transformers/blob/gpt-neo-localattention3-rp-b/src/transformers/models/gpt_neo/modeling_gpt_neo.py
...
from ....gpt_neo.configuration_gpt_neo import GPTNeoConfig
...
def to_gpu(x, config):
    model_dtypes = {"fp16": torch.float16,
...
def fixed_pos_embedding(dim=None, seq_len=None):
    ...
    sinusoid_inp = torch.einsum(
        'i , j -> i j', torch.arange(seq_len), inv_freq).float()
...
def apply_rotary_pos_emb(x, sincos, offset=0):
    sin, cos = map(lambda t: repeat(
        t[offset:x.shape[1]+offset, :], "n d -> () n () (d j)", j=2), sincos)
    ...
    def _attn(self, query, key, value, causal_mask, masked_bias, attn_dropout, attention_mask=None, head_mask=None, scale_attn=None, full_bf16=False):
        if not full_bf16 and query.dtype is torch.bfloat16:
            attn_weights = torch.matmul(
                query.float(), key.transpose(-1, -2).float())
        else:
        ...
        if not full_bf16 and value.dtype is torch.bfloat16:
            attn_output = torch.matmul(
                attn_weights.float(), value.float()).to(value.dtype)
        else:
...
class GPTNeoSelfAttention(nn.Module, GPTNeoAttentionMixin):
    def __init__(self, attention_type, config):
        ...
        self.out_proj = nn.Linear(
            self.embed_dim, self.embed_dim, bias=not config.jax)
        ...
        if self.rotary:
            sin, cos = fixed_pos_embedding(
                dim=self.rotary_dim, seq_len=max_positions)
            self.register_buffer("sin", sin)
            self.register_buffer("cos", cos)

        self.setup = False
        self.saved_attn_output = None
        self.saved_attn_gradients = None

    def save_attn_gradients(self, attn_gradients):
        #print("saving gradients" , attn_gradients)
        self.saved_attn_gradients = attn_gradients

    def save_attention_map(self, attn_output):
        # print(attn_output)
        self.saved_attn_output = attn_output

    def get_attn_gradients(self):
        # print()
        return self.saved_attn_gradients

    def get_attention_map(self):
        return self.saved_attn_output
    ...
    def forward(
        ...
        query = self._split_heads(
            query, self.num_heads, self.head_dim, self.rotary)
        key = self._split_heads(key, self.num_heads,
                                self.head_dim, self.rotary)
        ...
        if self.rotary:
            ...
            if self.rotary_dim < self.head_dim:
                ...
                k_rot = apply_rotary_pos_emb(
                    k_rot, (self.sin, self.cos), offset=offset).to(k_rot.dtype)
                q_rot = apply_rotary_pos_emb(
                    q_rot, (self.sin, self.cos), offset=offset).to(q_rot.dtype)

                key = torch.cat([k_rot, k_pass], dim=-1)
                query = torch.cat([q_rot, q_pass], dim=-1)
            elif self.rotary:
                key = apply_rotary_pos_emb(
                    key, (self.sin, self.cos), offset=offset).to(key.dtype)
                query = apply_rotary_pos_emb(
                    query, (self.sin, self.cos), offset=offset).to(query.dtype)
...
class GPTNeoAttention(nn.Module):
    ...
    def get_attn_gradients(self):
        return self.attention.get_attn_gradients()
        ...
        self.h = nn.ModuleList(
            [
                to_gpu(
                    GPTNeoBlock(
                        config,
                        # if FakeMagma is used, just stack up the last layer (i = config.num_layers - 1) n times
                        # until the desirable number of layers is reached
                        layer_id=i if i < config.num_layers else config.num_layers - 1  # FakeMagma patch
                    ),
                    config
                )
...
class GPTNeoForCausalLM(GPTNeoPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"h\.\d+\.attn\.attention\.masked_bias", r"h\.\d+\.attn\.masked_bias",
                                       r"h\.\d+\.attn\.attention\.scale_attn", r"h\.\d+\.attn\.attention\.(sin|cos)", r"lm_head\.weight", r"h\.\d+\.attn\.attention\.bias"]
    ...
    def __init__(self, config):
        ...
        self.jax = config.jax
        self.lm_head = to_gpu(
            nn.Linear(config.hidden_size, config.vocab_size, bias=self.jax), config)
        self.init_weights()
